...
const additionalJobTemplates: Partial<JobData>[] = [
...
function generateEnhancedJobs(count: number): JobData[] {
  const jobs: JobData[] = [];
  ...
  for (let i = 0; i < count; i++) {
    ...
    do {
      ...
      const employmentTypes: Array<'full-time' | 'part-time' | 'contract' | 'internship'> = ['full-time', 'part-time', 'contract', 'internship'];
      const senioryLevels: Array<'junior' | 'mid' | 'senior' | 'lead' | 'vp' | 'executive'> = ['junior', 'mid', 'senior', 'lead', 'vp', 'executive'];
      ...
      if (location.includes('San Francisco') || location.includes('New York') || location.includes('Zurich')) locationMultiplier = 1.3;
      else if (location.includes('London') || location.includes('Tokyo') || location.includes('Sydney')) locationMultiplier = 1.2;
      else if (location.includes('Dublin') || location.includes('Amsterdam') || location.includes('Toronto')) locationMultiplier = 1.1;
      else if (location.includes('Remote')) locationMultiplier = 1.05;
      else if (location.includes('India') || location.includes('Brazil') || location.includes('Philippines')) locationMultiplier = 0.6;
      else if (location.includes('Germany') || location.includes('Sweden') || location.includes('Canada')) locationMultiplier = 0.95;

      // Employment type salary adjustments
      let employmentMultiplier = 1.0;
      if (employmentType === 'part-time') employmentMultiplier = 0.6;
      else if (employmentType === 'contract') employmentMultiplier = 1.2; // Higher hourly rate
      else if (employmentType === 'internship') employmentMultiplier = 0.3;

      // Seniority adjustments
      let seniorityMultiplier = 1.0;
      if (seniorityLevel === 'junior') seniorityMultiplier = 0.8;
      else if (seniorityLevel === 'mid') seniorityMultiplier = 1.0;
      else if (seniorityLevel === 'senior') seniorityMultiplier = 1.3;
      else if (seniorityLevel === 'lead') seniorityMultiplier = 1.6;
      else if (seniorityLevel === 'vp') seniorityMultiplier = 2.0;
      else if (seniorityLevel === 'executive') seniorityMultiplier = 2.5;
