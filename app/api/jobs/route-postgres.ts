...
import { sql, Job, initializeDatabase } from '../../../lib/database-postgres';
...
async function handleSmartSearchQuery(
  ...
  if (sessionId && sessionId !== 'undefined') {
    jobs = await addMatchScores(jobs, sessionId);
...
async function handleRegularQuery(params: any) {
  ...
  if (sessionId && sessionId !== 'undefined') {
    const { rows: candidates } = await sql`SELECT * FROM candidates WHERE session_id = ${sessionId}`;
  ...
  const { rows: totalResult } = await sql.query(countQuery, queryValues);
  const total = parseInt(totalResult[0].total);

  queryValues.push(limit, offset);
  const jobsQuery = `${baseQuery} LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`;
  const { rows: jobs } = await sql.query(jobsQuery, queryValues);
  ...
  const processedJobs = jobs.map(job => ({
    ...
    remote_eligible: job.remote_eligible ? 1 : 0 // Convert boolean to number for consistency
  }));
...
async function addMatchScores(jobs: any[], sessionId: string) {
  const { rows: candidates } = await sql`SELECT * FROM candidates WHERE session_id = ${sessionId}`;
  const candidate = candidates[0];
