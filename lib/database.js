...
function getDatabase() {
    if (!db) {
        const dbPath = (0, path_1.join)(process.cwd(), 'career.db');
...
function seedSampleJobs() {
    const checkExisting = db.prepare('SELECT COUNT(*) as count FROM jobs').get();
    ...
    `);
        ...
        const insertMany = db.transaction((jobs) => {
            for (const job of jobs) {
                insertJob.run(job.title, job.description, job.location, job.company, job.department, job.seniority_level, job.required_skills, job.preferred_skills, job.salary_min, job.salary_max, job.employment_type, job.remote_eligible ? 1 : 0);
            }
        });
    ...
    (0, search_1.initializeSearchIndex)(processedJobs);
}
